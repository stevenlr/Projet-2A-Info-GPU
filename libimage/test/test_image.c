...
void test_image_new_delete()
{
	...
	for (i = 0; i < 3; ++i) {
		ut_assert("new image data", img->data[i] != NULL);
	}
...
void test_image_copy()
{
	...
	ut_assert("copy image data", memcmp(img1->data[0], img2->data[0], img1->width * img1->height * sizeof(uint8_t)) == 0);
...
void test_image_data()
{
	...
	ut_assert("image offet", Image_getOffset(img, 3, 4) == 67);
	...
	ut_assert("image set pixel", img->data[2][67] == 84);
